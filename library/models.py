...
class BorrowedBook(models.Model):
    ...
    def minutes_borrowed(self):
        ...
        if self.borrowed_date.date() == end_time.date():
            ...
            # Если книга была взята в один день и возвращена в другой,
            # считаем только минуты в день взятия и день возврата
            start_of_end_day = end_time.replace(
                hour=0, minute=0, second=0, microsecond=0
            )
            end_of_borrowed_day = self.borrowed_date.replace(
                hour=23, minute=59, second=59, microsecond=999999
            )
            return int(
                (end_of_borrowed_day - self.borrowed_date).total_seconds() // 60
            ) + int((end_time - start_of_end_day).total_seconds() // 60)
